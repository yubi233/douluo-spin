...
Vue 3 + TypeScript 的确定性命运模拟器。v0.3 使用结构化内容、显式 RNG、原子事件批次和 Process Manager 驱动人类魂师、魂兽、化形、魂环、剧情、神考、战后路线与终局。

当前正式内容包含 309 个池、2,601 个选项和 63 个实体。旧 `wheels.json` 与运行时中文文本解析器已从生产依赖中删除；2,500 个历史选项作为显式 `effects: []` 的纯叙事内容保留。
...
`npm run test:verify` 是完整门禁，依次执行内容审计、43 项 Vitest、生产类型检查与构建、生产架构扫描和 7 项真实 Chrome E2E。Playwright 使用独占端口 `4175`，证据位于 `test-results/iterations/v0.3/`。
...
最终发布认证固定为同一 commit、同一 `contentVersion` 的 20 批 x 20 份，共 400 份；目录为 `simulation-archives/<campaign>/<generation>/`。
