...
<template>
  <div v-if="open" class="modal-backdrop" role="presentation" @click.self="cancellable && emit('cancel')">
    <section class="start-dialog" role="dialog" aria-modal="true" aria-labelledby="start-title">
      ...
      <label class="seed-field"><span>模拟种子</span><input v-model="seed" /><button type="button" @click="seed = createSeed()"><Dices :size="17" /><span class="sr-only">随机种子</span></button></label>
    </section>
  </div>
</template>
